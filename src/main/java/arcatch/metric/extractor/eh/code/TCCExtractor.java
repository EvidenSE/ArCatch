...
public class TCCExtractor extends AbstractProcessor<CtClass<?>> {
	...
	public void process(CtClass<?> element) {
		if (Util.isValid(element)) {
			...
			double tryCyclomaticComplexity = 0.0;

			for (CtTry tryBlock : element.getElements(new TypeFilter<CtTry>(CtTry.class))) {
				tryCyclomaticComplexity += tryBlockCyclomaticComplexity(tryBlock);
			}

			Model.addMeasure(qualifiedName, new Measure(Metric.TCC, tryCyclomaticComplexity));
		}
	}
