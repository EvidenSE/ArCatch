...
public class RuleViolationAtUnitLevelReporter implements Reporter {

	@Override
	public void print() {
		StringBuffer report = new StringBuffer("Rule Id;Violation Kind;Violating Unit;Violation Label;First Unit;Second Unit\n");
		for (AntiErosionRule rule : Model.getFailingAntiErosionRules()) {
			...
			if (rule instanceof CallRule) {
				...
				if (violation.getOperationToOperationsViolations() != null && !violation.getOperationToOperationsViolations().isEmpty()) {
					for (BinaryRelation<Operation, Set<Operation>> violationInstance : violation.getOperationToOperationsViolations()) {
				...
				if (violation.getOperationToUnitsViolations() != null
						&& !violation.getOperationToUnitsViolations().isEmpty()) {
					for (BinaryRelation<Operation, Set<Unit>> violationInstance : violation.getOperationToUnitsViolations()) {
						buffer = new StringBuffer(firstPart);
						buffer.append(violationInstance.getLeft().getOwner().getQualifiedName());
						buffer.append(";");
						String secondPart = buffer.toString();
				...
				if (violation.getOperationToUnitsToOperationViolations() != null
					...
					for (TernaryRelation<Operation, Set<Unit>, Operation> violationInstance : violation.getOperationToUnitsToOperationViolations()) {
						buffer = new StringBuffer(firstPart);
						buffer.append(violationInstance.getLeft().getOwner().getQualifiedName());
						buffer.append(";");
						String secondPart = buffer.toString();
						if (!violationInstance.getMiddle().isEmpty()) {
							for (Unit unit : violationInstance.getMiddle()) {
								buffer = new StringBuffer(secondPart);
								buffer.append(violationInstance.getLeft());
								buffer.append(";");
								buffer.append(unit.getQualifiedName());
								buffer.append(";");
		...
		for (AntiDriftRule rule : Model.getFailingAntiDriftRules()) {
			StringBuffer buffer = new StringBuffer();
			buffer.append(rule.getId());
			buffer.append(";");
			buffer.append(RuleKing.Drift.getLabel());
			buffer.append(";");
			String firstPart = buffer.toString();
			AntiDriftRuleViolation violation = rule.getViolation();
			for(Unit unit : violation.getUnits()) {
				buffer = new StringBuffer(firstPart);
				buffer.append(unit.getQualifiedName());
				buffer.append(";");
